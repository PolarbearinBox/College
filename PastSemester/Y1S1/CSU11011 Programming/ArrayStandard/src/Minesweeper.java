...
public class Minesweeper {
	...
	public static void layMines(char[][] board, int numberOfMines)
	{
		Random generator = new Random();
		boolean mineLaid = false;
		for (int mine=0; (mine < numberOfMines); mine = mineLaid?mine+1:mine)
		{
			mineLaid = false;
			int mineRow = generator.nextInt(board.length);
			int mineColumn = generator.nextInt(board[mineRow].length);
	...
	// be careful not to access elements outside the array;
	{
		int numberOfMines = 0;
		for (int row = (passedRow == 0)? 0:passedRow-1; 
		row <= ((passedRow==board.length-1)?passedRow:passedRow+1);row++)
		{
			for (int column = (passedColumn == 0)?0: passedColumn-1;
			column<= ((passedColumn==board[row].length-1)?passedColumn:passedColumn+1);
			column++)
			{
				if (((row != passedRow) || (column != passedColumn)) 
